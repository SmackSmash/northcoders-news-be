...
describe('seed', () => {
    ...
    test('articles table has article_id column as a serial', () => {
      return db
        ...
        .then(({ rows: [column] }) => {
          ...
          expect(column.column_default).toBe("nextval('articles_article_id_seq'::regclass)");
        });
    });
    ...
    test('comments table has comment_id column as serial', () => {
      return db
        ...
        .then(({ rows: [column] }) => {
          ...
          expect(column.data_type).toBe('integer');
          expect(column.column_default).toBe("nextval('comments_comment_id_seq'::regclass)");
        });
    });
  ...
  describe('emojis table', () => {
    ...
    test('emojis table has an emoji_id column as serial', () => {
      return db
        .query(
          `SELECT *
            FROM information_schema.columns
        ...
        .then(({ rows: [column] }) => {
          ...
          expect(column.data_type).toBe('integer');
          expect(column.column_default).toBe("nextval('emojis_emoji_id_seq'::regclass)");
        });
    });

    test('emojis table has emoji_id column as primary key', () => {
      return db
        .query(
          `SELECT column_name
            FROM information_schema.table_constraints AS tc
    ...
    test('emojis table has emoji column as single character', () => {
      return db
        .query(
          `SELECT column_name, data_type, column_default
            FROM information_schema.columns
